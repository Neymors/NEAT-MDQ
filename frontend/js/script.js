...
// ===== CUSTOM CONFIRMATION NOTIFICATIONS (INYECCIÓN MINIMALISTA) =====
function mostrarConfirmacionCustom(fecha, alConfirmar) {
  ...
  confirmBox.style.transform = 'translateX(120%)';
  confirmBox.style.transition = 'transform 0.25s cubic-bezier(0.4, 0, 0.2, 1)';
  confirmBox.style.display = 'flex';
  confirmBox.style.flexDirection = 'column';
  confirmBox.style.gap = '0.75rem';
  confirmBox.style.minWidth = '280px';
  confirmBox.style.maxWidth = '360px';

  confirmBox.innerHTML = `
    <div class="font-mono text-xs uppercase tracking-widest font-bold" style="opacity:0.9;">
      ¿Confirmas la eliminación de los datos del ${fecha}?
    </div>
    ...
    <div style="display:flex;gap:0.5rem;justify-content:flex-end;">
      <button
        ...
        class="bento-btn"
        style="padding:0.25rem 0.5rem;font-size:0.65rem;background:transparent;"
      >
        CANCELAR
      </button>
      ...
      <button
        ...
        class="bento-btn"
        style="padding:0.25rem 0.5rem;font-size:0.65rem;border-color:var(--error);color:var(--error);"
      >
        CONFIRMAR
      </button>
    </div>
...
function mostrarNotificacionExito(mensaje) {
  ...
  toast.style.fontFamily = "'SF Mono', 'Fira Code', monospace";
...
const fechaFormateada = dateToDelete.split('-').reverse().join('/');
mostrarConfirmacionCustom(fechaFormateada, () => {
entries = entries.filter(e => e.date !== dateToDelete);
localStorage.setItem('stepData', JSON.stringify(entries));
updateUI();
mostrarNotificacionExito('Registro eliminado correctamente');
});
};
...
return  `<tr style=` `"transition:background 0.15s;` `"` `> ` `<td style=` `"font-family:monospace;font-size:0.75rem;padding:0.5rem;` `"` `>${formattedDate}` `</td` `> ` `<td style=` `"font-family:monospace;font-size:0.75rem;text-align:right;padding:0.5rem;color:${goalReached ? 'var(--success)' : 'var(--text-muted)'};font-weight:${goalReached ? '700' : '400'};` `"` `>${e.steps.toLocaleString()}` `</td` `> ` `<td style=` `"text-align:center;padding:0.5rem;display:flex;gap:0.5rem;justify-content:center;` `"` `> ` `<button class=` `"action-btn edit-entry-btn` `" data-date=` `"${e.date}` `" title=` `"Editar métrica` `" style=` `"background:transparent;border:none;cursor:pointer;color:#10b981;font-size:0.75rem;padding:0.25rem;transition:all 0.15s;border-radius:var(--radius-sm);` `"` `>` `<i class=` `"fas fa-pencil-alt` `"` `>` `</i` `>` `</button` `> ` `<button class=` `"delete-entry-btn` `" data-date=` `"${e.date}` `" title=` `"Eliminar métrica` `" style=` `"background:transparent;border:none;cursor:pointer;color:#ef4444;font-size:0.75rem;padding:0.25rem;transition:all 0.15s;border-radius:var(--radius-sm);` `"` `>` `<i class=` `"fas fa-trash-alt` `"` `>` `</i` `>` `</button` `> ` `</td` `> ` `</tr` `>` ;
}).join('');
}
...
return  `<div style=` `"border:1px solid var(--border);padding:0.75rem;background:var(--surface-accent);border-radius:var(--radius-sm);` `"` `> ` `<div style=` `"display:flex;justify-content:space-between;margin-bottom:0.5rem;font-size:0.7rem;` `"` `> ` `<span style=` `"font-weight:700;color:${done ? 'var(--accent)' : 'var(--text-main)'};` `"` `>` `<i class=` `"fas ${m.icon}` `" style=` `"margin-right:0.4rem;` `"` `>` `</i` `>${m.name}` `</span` `> ` `<span style=` `"font-family:monospace;color:var(--text-muted);` `"` `>${percent}%` `</span` `> ` `</div` `> ` `<div class=` `"progress-rail` `" style=` `"height:4px;margin-bottom:0.5rem;` `"` `>` `<div class=` `"progress-track` `" style=` `"width:${percent}%` `"` `>` `</div` `>` `</div` `> ` `<div style=` `"display:flex;justify-content:space-between;font-size:0.65rem;font-family:monospace;color:var(--text-muted);` `"` `> ` `<span` `>${currentKm} / ${m.km} km` `</span` `> ` `<span style=` `"max-width:60%;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;` `"` `>${m.desc}` `</span` `> ` `</div` `> ` `</div` `>` ;
}).join('');
}
